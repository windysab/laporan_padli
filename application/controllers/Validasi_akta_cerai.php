...
class Validasi_akta_cerai extends CI_Controller
{
	...
	public function index()
	{
		...
		$data += [
			...
			'selected_nomor_akta' => $nomor_akta,
		];

		view_load('v_validasi_akta_cerai', $data);
	}
	...
	public function export_excel()
	{
		...
		foreach ($data as $item) {
			if ($mode === 'cek_nomor') {
				$row = ['VALID / ASLI', $item->nomor_akta_cerai, $item->no_seri_akta_cerai, $item->tgl_akta_cerai, $item->nomor_perkara, $item->jenis_perkara_nama, $item->penggugat, $item->tergugat, $item->tanggal_putusan, $item->tanggal_bht];
			} elseif ($mode === 'terlambat') {
				$row = [$no++, $item->nomor_perkara, $item->jenis_perkara_nama, $item->penggugat, $item->tergugat, $item->tanggal_putusan, $item->tanggal_bht, $item->tgl_akta_cerai, $item->selisih_hari, $item->status_keterlambatan];
			} else {
				$row = [$no++, $item->nomor_perkara, $item->jenis_perkara_nama, $item->penggugat, $item->tergugat, $item->tanggal_putusan, $item->tanggal_bht, $item->nomor_akta_cerai, $item->no_seri_akta_cerai, $item->tgl_akta_cerai, $item->catatan_validasi];
			}
			fputcsv($output, $row, ';');
		}
		fclose($output);
